...
-(IBAction)back_Pressed{
	...
	if (timeLeft == 0) {
		...
		profile.LoggedinUser.EquationGame2Average = 
		((profile.LoggedinUser.EquationGame2Average*(float)profile.LoggedinUser.EquationGame2Played) + (float)totalScore) / ((float)profile.LoggedinUser.EquationGame2Played+1);
...
-(void)parse_Question {
	...
	//multiplication question
		...
		do {
			number1 = 1 + arc4random() % 9;
			number2 = 1 + arc4random() % 9;
		} while ((number1 == 2 && number2 == 2) || (number1 == 1 && number2 == 1));
		Answer = number1 * number2;
	}
	//division question 	
	else if(operand == 4){
