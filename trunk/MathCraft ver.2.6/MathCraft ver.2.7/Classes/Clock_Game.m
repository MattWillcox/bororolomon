...
-(IBAction) returnpreviouspage{
	...
	if (timeleft == 0) {
		...
		if (score > profile.LoggedinUser.ClockGame1Score) {
			profile.LoggedinUser.ClockGame1Score = score;
		}
		
		profile.LoggedinUser.ClockGame1Average = 
		((profile.LoggedinUser.ClockGame1Average*(float)profile.LoggedinUser.ClockGame1Played) + (float)score) / ((float)profile.LoggedinUser.ClockGame1Played+1);
		profile.LoggedinUser.ClockGame1Played++;
