...
-(void)update_Label { //Updates the timer every 1 second, if timer reads 0 game ends
	...
	if (timeLeft != 0) {
		timeLeft = timeLeft--;
		if (timeLeft == 30){
			UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Game type 2 will now begin"
															message:@"Select the correct operator missing from the equation." 
														   delegate:nil 
			...
			switch(Operand2){
				case 1: 
					...
					answerLabel.text = [NSString stringWithFormat:@"%ld", Answer];
					break;
				case 2:
					while (Num1 < Num2 || Num1/Num2 == 2 || Num1/Num2 == 2) {
						Num1 = 1 + arc4random() % 9;
						Num2 = 1 + arc4random() % 9;
					}
					Answer = Num1 - Num2; 
