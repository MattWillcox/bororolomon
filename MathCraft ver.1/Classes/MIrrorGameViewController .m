...
-(IBAction)answer_Push:(id)sender {
	...
	if (timeLeft != 0) {
	if ([button1 backgroundImageForState:UIControlStateNormal] == [button6 backgroundImageForState:UIControlStateNormal] && 
		[button2 backgroundImageForState:UIControlStateNormal] == [button5 backgroundImageForState:UIControlStateNormal] &&
		[button3 backgroundImageForState:UIControlStateNormal] == [button8 backgroundImageForState:UIControlStateNormal] &&
		[button4 backgroundImageForState:UIControlStateNormal] == [button7 backgroundImageForState:UIControlStateNormal]) {
		score++;
		scoreLabel.text = [NSString stringWithFormat:@"%ld", score];
	}
	else if (([button1 backgroundImageForState:UIControlStateNormal] != [button6 backgroundImageForState:UIControlStateNormal] || 
			 [button2 backgroundImageForState:UIControlStateNormal] != [button5 backgroundImageForState:UIControlStateNormal] ||
			 [button3 backgroundImageForState:UIControlStateNormal] != [button8 backgroundImageForState:UIControlStateNormal] ||
			 [button4 backgroundImageForState:UIControlStateNormal] != [button7 backgroundImageForState:UIControlStateNormal]) && 
	score != 0){
